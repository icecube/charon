...
def NFW(r,rs=24.42,rhos=0.184,gamma=1):
	return rhos/((r/rs)**gamma*(1.+(r/rs))**(3-gamma))


def Einasto(r,rs=28.44,rhos=0.033,alpha=0.17):
	return rhos*np.exp(-(2./alpha)*((r/rs)**alpha-1.))


def Burkert(r,rs=12.67,rhos=0.712):
	return rhos/((1.+r/rs)*(1.+r**2/rs**2))


def Isothermal(r,rs=4.38,rhos=1.387):
	return rhos/(1.+r**2/rs**2)

#Zhao, H. 1996, MNRAS, 278, 488
#Moore: https://arxiv.org/pdf/astro-ph/0402267.pdf
def Zhao(r,rs=30.28,rhos=0.105,alpha=1.,beta=3.,gamma=1.16):
...
class J:
	...
	#return jfactor with unit GeV^2/cm^5 for annihilation and GeV/cm^2 for decay
		l = np.linspace(0.,l_max(theta,self.R,self.d),1001)
	...
	def JIntegral(self,theta_min,theta_max):
		...
		cos_mid    = (cos[1:]+cos[:-1])/2.
		diff       = np.diff(cos)
		return 2*np.pi*sum([self.Jtheta(np.arccos(cos_mid[i]))*(diff[i]) for i in range(len(diff))])
