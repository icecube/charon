...
def DMSunCaptureRateGould(DM_mass,DM_cs,DM_rho=0.3,vel_rot=220.,model=None,n =None,mass_num=None,mass_gr_per_mol=None,eps=None):
    ...
    if any(e!= None for e in [n,mass_num,eps,mass_gr_per_mol]):
        ...
        eps             = [0.772,0.209,3.87e-3,9.4e-4,8.55e-3,1.51e-3,7.39e-4,8.13e-4,4.65e-4,1.46e-3]       # relative aboundances in the Sun from Table 8 Jungman p.131    
        n               = len(elements)
...
def DMSunAnnihilationRateGould(DM_mass,DM_cs,model=None,n =None,mass_num=None,mass_gr_per_mol=None,eps=None,DM_rho=0.3,vel_rot=220.):
    """Calculate annihilation rate
    """
    return DMSunCaptureRateGould(DM_mass,DM_cs,model=model,n =n,mass_num=mass_num,mass_gr_per_mol=mass_gr_per_mol,eps=eps,DM_rho=DM_rho,vel_rot=vel_rot)/2.


def DMSunCaptureRateHalzen(DM_mass,DM_cs,DM_rho=0.3,vel_rot=270.):
    ...
    return 3.35*1.0e20*pc.sec**-1*(DM_rho/(0.3*pc.GeV/pc.cm**3))*((270.0*pc.km/pc.sec)/vel_rot)**3*(100.0*pc.GeV/DM_mass)**2*(DM_cs/(1.0e-6*pc.picobarn))*pc.sec


def DMSunAnnihilationRateHalzen(DM_mass,DM_cs,DM_rho=0.3,vel_rot=270.):
    """Calculate annihilation rate
    """    
    return DMSunCaptureRateHalzen(DM_mass,DM_cs,DM_rho=DM_rho,vel_rot=vel_rot)/2.
...
def DMCaptureJungmanSI(DM_mass,DM_cs,location='Sun',DM_rho=0.3,vel_rot=270.0):
    ...
    if   location == 'Sun':
        ...
        mass_num        = [1.0,4.0,12.0,14.0,16.0,20.0,24.0,28.0,32.0,56.0]                                 # A : mass number
        mass_gr_per_mol = [1.0079,4.0026,12.0107,14.0067,15.9994,20.1797,24.3051,28.0855,32.0655,55.8452]   # gr mol^-1
        eps             = [0.772,0.209,3.87e-3,9.4e-4,8.55e-3,1.51e-3,7.39e-4,8.13e-4,4.65e-4,1.46e-3]       # relative aboundances in the Sun from Table 8 Jungman p.130
        ...
        elements        = {0:'O',1:'Si',2:'Mg',3:'Fe',4:'Ca',5:'P',6 :'Na',7:'S',8:'Ni'}               # names
        n               = len(elements)
        mass_num        = [16.0,28.0,24.0,56.0,40.0,30.0,23.0,32.0,59.0]
        mass_gr_per_mol = [15.9994,28.0855,24.305,55.8452,40.078,30.973761,22.98977,32.0655,58.6934]
    ...
    nuclear_term = [(((mass_num[i]*reduce_mass(DM_mass,mass_eV[i])/reduce_mass(DM_mass,p_mass))**2)/(mass_eV[i]/pc.GeV))*eps[i]*phi[i]*JungmanFormFactor(i,DM_mass)*kinematical_supression_factor(mass_eV[i]/DM_mass) for i in range(n)]    
    
    C_c = (c*pc.sec**-1)*(DM_rho/(0.3*pc.GeV/pc.cm**3))*((270*pc.km*pc.sec**-1)/vel_rot)*(1.0*pc.GeV/DM_mass)*(DM_cs/(1.0e-40*pc.cm**2))*sum(nuclear_term)
...
def DMAnnihilationJungmanSI(DM_mass,DM_cs,DM_rho=0.3,vel_rot=270.0):
    """Calculate annihilation rate
    """
    return DMCaptureJungmanSC(DM_mass,DM_cs,DM_rho=DM_rho,vel_rot=vel_rot)/2.


def DMCaptureJungmanSD(DM_mass,DM_cs,DM_rho=0.3,vel_rot=270.):
    """ Calculates DM annihilation rate in the Sun using Jungman/Kamionkowski aproximated formula
    ...
    C_c = (1.3e25*pc.sec**-1)*(DM_rho/(0.3*pc.GeV/pc.cm**3))*((270*pc.km*pc.sec**-1)/vel_rot)*(1.0*pc.GeV/DM_mass)*(DM_cs/(1.0e-40*pc.cm**2))*kinematical_supression_factor(DM_mass/mass_eV)
    
    return C_c*pc.sec


def DMAnnihilationJungmanSD(DM_mass,DM_cs,DM_rho=0.3,vel_rot=270.):
    """Calculate annihilation rate
    """
    return DMCaptureJungmanAX(DM_mass,DM_cs,DM_rho=DM_rho,vel_rot=vel_rot)/2.
