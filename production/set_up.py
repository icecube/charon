...
if channel in ("nuenue", "numunumu", "nutaunutau"):
    binning = np.linspace(0.0, mass, bins + 1)
    bin_center = (binning[1:] + binning[:-1]) / 2.0
    for i in range(6):
        flux_path = "./{}/{}_{}_{}_{}-{}.dat".format(
            location, channel, mass, location, process, i
        )
        f = open(flux_path, "w")
        if i == nu_number[channel] or i == nu_number[channel] + 1:
            ...
            if process == "ann":
                for j in range(bins):
                    if j < bins - 1:
                        f.write(
                            "{:.4E}".format(bin_center[j]) + "\t" + "0.0000e+00" + "\n"
                        )
                    else:
                        non_zero = 1.0 / diff[-1]
                        f.write(
                            "{:.4E}".format(bin_center[j])
                            + "\t"
                            + "{}".format(non_zero)
                            + "\n"
                        )
